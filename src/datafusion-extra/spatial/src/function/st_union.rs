...
pub fn st_union() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(UnionUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_union".to_owned()],
    }))
}
...
impl ScalarUDFImpl for UnionUdf {
                        ...
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                    (Ok(geom1), Ok(geom2)) => {
                                        geom1.union(&geom2).and_then(|geom| geom.to_ewkb()).ok()
                                    }
                                    _ => None,
                                }
                            }
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .and_then(|geom1| geom1.union(&geom2))
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .and_then(|geom2| geom1.union(&geom2))
                                                .and_then(|geom| geom.to_ewkb())
                                                .ok()
                                        })
                                    })
