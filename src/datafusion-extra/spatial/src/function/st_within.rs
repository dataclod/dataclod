...
pub fn st_within() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(WithinUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_within".to_owned()],
    }))
}
...
impl ScalarUDFImpl for WithinUdf {
                        ...
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                    (Ok(geom1), Ok(geom2)) => geom1.st_within(&geom2),
                                    _ => None,
                                }
                            }
                            _ => None,
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .ok()
                                                .and_then(|geom1| geom1.st_within(&geom2))
                                        })
                                    })
                                    .collect();
                                Arc::new(result)
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .ok()
                                                .and_then(|geom2| geom1.st_within(&geom2))
                                        })
                                    })
                                    .collect();
                                Arc::new(result)
