...
pub fn st_simplifypreservetopology() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(SimplifyPreserveTopologyUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_simplifypreservetopology".to_owned()],
    }))
}

#[derive(Debug, PartialEq, Eq, Hash)]
struct SimplifyPreserveTopologyUdf {
    signature: Signature,
    aliases: Vec<String>,
}

impl ScalarUDFImpl for SimplifyPreserveTopologyUdf {
                        ...
                        match opt {
                            (Some(wkb), Some(tolerance)) => {
                                Geometry::new_from_wkb(wkb)
                                    .and_then(|geom| geom.topology_preserve_simplify(tolerance))
                        ...
                        let result: BinaryViewArray = wkb_arr
                            ...
                            .map(|opt| {
                                opt.and_then(|wkb| {
                                    Geometry::new_from_wkb(wkb)
                                        .and_then(|geom| {
                                            geom.topology_preserve_simplify(*tolerance)
                                        })
                                        .and_then(|geom| geom.to_ewkb())
                                        .ok()
                                })
