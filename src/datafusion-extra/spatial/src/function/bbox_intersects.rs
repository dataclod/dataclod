...
pub fn bbox_intersects() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(BBoxIntersectsUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["bbox_intersects".to_owned()],
    }))
}

#[derive(Debug, PartialEq, Eq, Hash)]
struct BBoxIntersectsUdf {
    signature: Signature,
    aliases: Vec<String>,
}

impl ScalarUDFImpl for BBoxIntersectsUdf {
                        ...
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                    (Ok(geom1), Ok(geom2)) => geom1.bbox_overlaps(&geom2).ok(),
                                    _ => None,
                                }
                            }
                            _ => None,
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .and_then(|geom1| geom1.bbox_overlaps(&geom2))
                                                .ok()
                                        })
                                    })
                                    .collect();
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .and_then(|geom2| geom1.bbox_overlaps(&geom2))
                                                .ok()
                                        })
                                    })
                                    .collect();
