...
pub fn st_centroid() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(CentroidUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_centroid".to_owned()],
    }))
}
...
impl ScalarUDFImpl for CentroidUdf {
                    ...
                    .map(|opt| {
                        opt.and_then(|wkb| {
                            Geometry::new_from_wkb(wkb)
                                .and_then(|geom| geom.get_centroid())
