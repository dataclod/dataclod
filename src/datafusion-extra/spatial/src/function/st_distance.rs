...
use arrow::array::{Array, AsArray, Float64Array, new_null_array};
...
pub fn st_distance() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(DistanceUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_distance".to_owned()],
    }))
}
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> DFResult<ColumnarValue> {
        ...
        match (&args[0], &args[1]) {
            (ColumnarValue::Array(wkb_arr1), ColumnarValue::Array(wkb_arr2)) => {
                ...
                let result: Float64Array = wkb_arr1
                    .iter()
                    .zip(wkb_arr2.iter())
                    .map(|opt| {
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                    (Ok(geom1), Ok(geom2)) => geom1.distance(&geom2).ok(),
                                    _ => None,
                                }
                            }
                            _ => None,
                ...
                let result = match wkb_opt2 {
                    Some(wkb2) => {
                        match Geometry::new_from_wkb(wkb2) {
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .and_then(|geom1| geom1.distance(&geom2))
                                                .ok()
                                        })
                                    })
                                    .collect();
                            ...
                            _ => new_null_array(&DataType::Float64, wkb_arr1.len()),
                        }
                    }
                    None => new_null_array(&DataType::Float64, wkb_arr1.len()),
                };
                ...
                let result = match wkb_opt1 {
                    Some(wkb1) => {
                        match Geometry::new_from_wkb(wkb1) {
                            Ok(geom1) => {
                                let wkb_arr2 = wkb_arr2.as_binary_view();
                                let result: Float64Array = wkb_arr2
                                    .iter()
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .and_then(|geom2| geom1.distance(&geom2))
                                                .ok()
                                        })
                                    })
                                    .collect();
                            ...
                            _ => new_null_array(&DataType::Float64, wkb_arr2.len()),
                        }
                    }
                    None => new_null_array(&DataType::Float64, wkb_arr2.len()),
                };
                Ok(ColumnarValue::Array(result))
            }
            (
