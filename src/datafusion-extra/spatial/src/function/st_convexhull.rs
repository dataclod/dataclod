...
pub fn st_convexhull() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(ConvexHullUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_convexhull".to_owned()],
    }))
}

#[derive(Debug, PartialEq, Eq, Hash)]
struct ConvexHullUdf {
    signature: Signature,
    aliases: Vec<String>,
}

impl ScalarUDFImpl for ConvexHullUdf {
                    ...
                    .map(|opt| {
                        opt.and_then(|wkb| {
                            Geometry::new_from_wkb(wkb)
                                .and_then(|geom| geom.convex_hull())
