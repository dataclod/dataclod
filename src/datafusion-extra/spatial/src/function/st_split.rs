...
pub fn st_split() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(SplitUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_split".to_owned()],
    }))
}
        ...
        match (&args[0], &args[1]) {
                ...
                let result: BinaryViewArray = wkb_arr1
                    ...
                    .map(|opt| {
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (
                                    lwgeom::LWGeom::from_ewkb(wkb1),
                                    lwgeom::LWGeom::from_ewkb(wkb2),
                                ) {
                                    (Ok(geom1), Ok(geom2)) => geom1.split(&geom2).as_ewkb().ok(),
                                    _ => None,
                                }
                            }
                            _ => None,
            ...
            (
                ...
                let result = match wkb_opt2 {
                    Some(wkb2) => {
                        match lwgeom::LWGeom::from_ewkb(wkb2) {
                            Ok(geom2) => {
                                let wkb_arr1 = wkb_arr1.as_binary_view();
                                let result: BinaryViewArray = wkb_arr1
                                    .iter()
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            lwgeom::LWGeom::from_ewkb(wkb1)
                                                .and_then(|geom1| geom1.split(&geom2).as_ewkb())
                                                .ok()
                                        })
                                    })
                                    .collect();
            ...
            (
                ...
                let result = match wkb_opt1 {
                    Some(wkb1) => {
                        match lwgeom::LWGeom::from_ewkb(wkb1) {
                            Ok(geom1) => {
                                let wkb_arr2 = wkb_arr2.as_binary_view();
                                let result: BinaryViewArray = wkb_arr2
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            lwgeom::LWGeom::from_ewkb(wkb2)
                                                .and_then(|geom2| geom1.split(&geom2).as_ewkb())
                                                .ok()
                                        })
                                    })
                                    .collect();
