...
pub fn st_difference() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(DifferenceUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_difference".to_owned()],
    }))
}
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .and_then(|geom1| geom1.difference(&geom2))
                                                .and_then(|geom| geom.to_ewkb())
                                                .ok()
                                        })
                                    })
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .and_then(|geom2| geom1.difference(&geom2))
                                                .and_then(|geom| geom.to_ewkb())
                                                .ok()
                                        })
                                    })
