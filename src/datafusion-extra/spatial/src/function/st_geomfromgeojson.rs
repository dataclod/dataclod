...
pub fn st_geomfromgeojson() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(GeomFromGeoJSONUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_geomfromgeojson".to_owned()],
    }))
}

#[derive(Debug, PartialEq, Eq, Hash)]
struct GeomFromGeoJSONUdf {
    signature: Signature,
    aliases: Vec<String>,
}
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> DFResult<ColumnarValue> {
        ...
        match &args[0] {
            ColumnarValue::Array(arr) => {
                let geojson_arr = arr.as_string_view();
                let result: BinaryViewArray = geojson_arr
                    .iter()
                    .map(|opt| {
                        opt.and_then(|geojson| {
                            Geometry::new_from_geojson(geojson)
            ...
            ColumnarValue::Scalar(ScalarValue::Utf8View(opt)) => {
                let result = opt.as_ref().and_then(|geojson| {
                    Geometry::new_from_geojson(geojson)
                        .and_then(|geom| geom.to_ewkb())
                        .ok()
                });
