...
pub fn st_translate() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(TranslateUdf {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> DFResult<ColumnarValue> {
        ...
        match (&args[0], &args[1], &args[2]) {
            (
                ...
                ColumnarValue::Array(delta_x_arr),
                ColumnarValue::Array(delta_y_arr),
            ) => {
                let wkb_arr = wkb_arr.as_binary_view();
                let delta_x_arr = delta_x_arr.as_primitive::<Float64Type>();
                let delta_y_arr = delta_y_arr.as_primitive::<Float64Type>();

                let result: BinaryViewArray = multizip((wkb_arr, delta_x_arr, delta_y_arr))
                    .map(|opt| {
                        match opt {
                            (Some(wkb), Some(delta_x), Some(delta_y)) => {
                                Geometry::new_from_wkb(wkb)
                                    .and_then(|geom| geom.st_translate(delta_x, delta_y))
            ...
            (
                ColumnarValue::Array(wkb_arr),
                ColumnarValue::Scalar(ScalarValue::Float64(Some(delta_x))),
                ColumnarValue::Scalar(ScalarValue::Float64(Some(delta_y))),
            ) => {
                let wkb_arr = wkb_arr.as_binary_view();
            ...
            (
                ColumnarValue::Scalar(ScalarValue::BinaryView(wkb)),
                ColumnarValue::Scalar(ScalarValue::Float64(delta_x)),
                ColumnarValue::Scalar(ScalarValue::Float64(delta_y)),
            ) => {
                let result = match (wkb, delta_x, delta_y) {
                    (Some(wkb), Some(delta_x), Some(delta_y)) => {
                        Geometry::new_from_wkb(wkb)
                            .and_then(|geom| geom.st_translate(*delta_x, *delta_y))
                            .and_then(|geom| geom.to_ewkb())
