...
pub fn st_touches() -> Arc<ScalarUDF> {
    Arc::new(ScalarUDF::new_from_impl(TouchesUdf {
        signature: Signature::user_defined(Volatility::Immutable),
        aliases: vec!["st_touches".to_owned()],
    }))
}
                        ...
                        match opt {
                            (Some(wkb1), Some(wkb2)) => {
                                match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                    (Ok(geom1), Ok(geom2)) => geom1.st_touches(&geom2),
                                    _ => None,
                                }
                            }
                            _ => None,
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb1| {
                                            Geometry::new_from_wkb(wkb1)
                                                .ok()
                                                .and_then(|geom1| geom1.st_touches(&geom2))
                                        })
                                    })
                                    .collect();
                                Arc::new(result)
                                    ...
                                    .map(|opt| {
                                        opt.and_then(|wkb2| {
                                            Geometry::new_from_wkb(wkb2)
                                                .ok()
                                                .and_then(|geom2| geom1.st_touches(&geom2))
                                        })
                                    })
                                    .collect();
                                Arc::new(result)
