...
/// Filter specification that determines whether a certain sqllogictest record
/// in a certain file should be filtered. In order for a [`Filter`] to match a
/// test case:
///
/// - The test must belong to a file whose absolute path contains the
///   `file_substring` substring.
/// - If a `line_number` is specified, the test must be declared in that same
///   line number.
///
/// If a [`Filter`] matches a specific test case, then the record is executed,
/// if there's no match, the record is skipped.
///
/// Filters can be parsed from strings of the form `<file_name>:line_number`.
/// For example, `foo.slt:100` matches any test whose name contains `foo.slt`
/// and the test starts on line number 100.
#[derive(Debug, Clone)]
pub struct Filter {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() == 2 {
            match parts[0].parse::<u32>() {
                Ok(line) => {
                    Ok(Filter {
                        file_substring: parts[1].to_owned(),
...
/// path can be skipped.
///
/// - If there's at least 1 filter whose file name is a substring of the
///   provided path, it returns true.
...
/// Determines whether a certain sqllogictest record should be skipped given the
/// provided filters.
///
/// If there's at least 1 matching filter, or the filter list is empty, it
/// returns false.
///
/// There are certain records that will never be skipped even if they are not
/// matched by any filters, like CREATE TABLE, INSERT INTO, DROP or SELECT *
/// INTO statements, as they populate tables necessary for other tests to work.
pub fn should_skip_record<D: AsyncDB>(record: &Record<D::ColumnType>, filters: &[Filter]) -> bool {
    if filters.is_empty() {
        return false;
    }
