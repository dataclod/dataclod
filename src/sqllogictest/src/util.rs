...
/// fail if it is not If particular test wants to cover trailing whitespace on a
/// value, it should project additional non-whitespace column on the right.
...
pub fn df_value_validator(
    normalizer: Normalizer, actual: &[Vec<String>], expected: &[String],
) -> bool {
...
mod tests {
    ...
    fn ignore_marker_does_not_skip_leading_text() {
        ...
        let actual = vec![vec!["foo bar baz".to_owned()]];
        let expected = vec!["bar <slt:ignore>".to_owned()];
