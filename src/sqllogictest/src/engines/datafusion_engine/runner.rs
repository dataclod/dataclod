...
use super::{DCSqlLogicTestError, normalize};
...
impl DataClod {
    pub fn new(ctx: QueryContext, relative_path: PathBuf, pb: ProgressBar) -> Self {
        Self {
            ctx,
            relative_path,
            pb,
    ...
    pub fn with_currently_executing_sql_tracker(
        self, currently_executing_sql_tracker: CurrentlyExecutingSqlTracker,
    ) -> Self {
        Self {
...
#[async_trait]
impl sqllogictest::AsyncDB for DataClod {
    type ColumnType = DFColumnType;
    type Error = DCSqlLogicTestError;
    ...
    /// [`DataClod`] calls this function to perform sleep.
    ///
    /// The default implementation is `std::thread::sleep`, which is universal
    /// to any async runtime but would block the current thread. If you are
    /// running in tokio runtime, you should override
...
async fn run_query(ctx: &QueryContext, sql: impl Into<String>) -> Result<DFOutput> {
    let df = ctx
        .sql(sql.into().as_str())
        .await
        .map_err(|e| DCSqlLogicTestError::Other(e.to_string()))?;
    ...
    let rows = normalize::convert_batches(&schema, results)?;
