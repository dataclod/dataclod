...
use super::error::{DCSqlLogicTestError, Result};
...
pub fn convert_batches(schema: &Schema, batches: Vec<RecordBatch>) -> Result<Vec<Vec<String>>> {
    ...
    for batch in batches {
        ...
        if !schema.contains(&batch.schema()) {
            return Err(DCSqlLogicTestError::DataFusion(internal_datafusion_err!(
        ...
        // Convert a single batch to a `Vec<Vec<String>>` for comparison, flatten
        // expanded rows, and normalize each.
...
static WORKSPACE_ROOT: LazyLock<object_store::path::Path> = LazyLock::new(|| {
    ...
    let sanitized_workplace_root = if cfg!(windows) {
        ...
        // `/datafusion/datafusion/testing/data/csv/aggregate_test_100.csv`. The
        // default windows delimiter is `\`, so the workplace path is
...
pub fn cell_to_string(col: &ArrayRef, row: usize) -> Result<String> {
    if !col.is_valid(row) {
        ...
        match col.data_type() {
            DataType::Null => Ok(NULL_STR.to_owned()),
            ...
            DataType::LargeUtf8 => {
                Ok(varchar_to_str(get_row_value!(
            ...
            _ => {
                let mut datafusion_format_options = datafusion::config::FormatOptions::default();

                datafusion_format_options.set("null", "NULL").unwrap();

                let arrow_format_options: datafusion::arrow::util::display::FormatOptions =
        ...
        .map_err(DCSqlLogicTestError::Arrow)
    }
}
    ...
    columns
        ...
        .map(|data_type| {
            match data_type {
                ...
                | DataType::Float64
                | DataType::Decimal128(..)
                | DataType::Decimal256(..) => DFColumnType::Float,
                ...
                DataType::Timestamp(..) => DFColumnType::Timestamp,
