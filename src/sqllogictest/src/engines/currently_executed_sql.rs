...
pub struct CurrentlyExecutingSqlTracker {
    ...
    /// It DOES NOT hold the lock for the duration of query execution and only
    /// execute the lock when updating the currently executed SQL statement
    /// to allow for saving the last executed SQL in case of a crash.
...
impl CurrentlyExecutingSqlTracker {
    ...
    ///
    /// We are not returning a guard that will automatically remove the SQL
    /// statement when dropped. as on panic the drop can be called, and it
    /// will remove the SQL statement before we can log it.
    ...
    /// Remove the currently executed SQL statement by the provided key that was
    /// returned by [`Self::set_sql`].
